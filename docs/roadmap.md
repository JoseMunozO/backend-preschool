...
- [ ] Crear endpoint para gestionar `ChargeType` (tipos de cargo: mensualidad, comedor, excursion, etc.) (2026-08-24, genuinamente abierto). Hoy `ChargeType` solo tiene `GET /api/payments/charge-types` — no hay `POST`/`PUT`, y los 4 tipos base (con sus montos, ver seccion C) solo existen en el seed de desarrollo (`docker/mysql/init/01-base-schema.sql`), no en ninguna migracion de Flyway. En una base de datos de produccion limpia no habria ningun `ChargeType`, asi que la generacion automatica de mensualidad (ver seccion C) no generaria nada hasta crearlos a mano por SQL. Falta un endpoint para que admin/finanzas puedan crear y ajustar precios (ej. la mensualidad) desde la aplicacion en vez de tocar la base de datos directamente.
