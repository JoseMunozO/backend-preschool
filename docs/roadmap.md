...
- [x] Descuentos: recalculo instantaneo de cargos abiertos (2026-08-24/25): reportado por el cliente probando en vivo contra un estudiante real (Lucas Andersson) — crear o desactivar un descuento no cambiaba el cargo pendiente del mes, habia que esperar al proximo ciclo de generacion. Causa: el calculo del descuento solo ocurria al generar el cargo (una sola vez por mes, nunca se re-ejecutaba). Fix: `createDiscount`/`deactivateDiscount` ahora recalculan de inmediato todos los cargos abiertos (`PENDING`/`PARTIALLY_PAID`/`OVERDUE`) del estudiante. Un segundo bug se coló en ese mismo fix (mismo dia, detectado probando contra el estudiante real antes de que el cliente lo viera): el recalculo buscaba el descuento vigente usando la fecha de inicio del periodo de facturacion (el dia 1) en vez de hoy, asi que un descuento creado a mitad de mes nunca calificaba como vigente para el cargo ya generado — corregido para usar la fecha actual. Verificado end-to-end contra Docker real con datos reales (no solo con tests): el cargo de Lucas Andersson bajo de RD$6,000 a RD$3,000 al instante con un 50% de descuento activo.
- [x] Descuentos: dos tipos de vigencia + tope por retiro del estudiante (2026-08-25): pedido del cliente — un descuento "instantaneo" que aplique ya mismo y solo dure el ciclo de facturacion en curso (sin tener que desactivarlo despues), y otro "programado" con fecha de inicio/fin elegida por el usuario (el comportamiento que ya existia). Nuevo campo `durationType` (`INSTANT`/`SCHEDULED`, migracion `V20`, descuentos existentes migrados como `SCHEDULED` para no cambiar su comportamiento). `INSTANT` ignora cualquier fecha que mande el cliente y calcula `validFrom=hoy`, `validUntil=fin del mes en curso` en el servidor; `SCHEDULED` exige `validFrom` (400 si falta). Ademas, todo descuento queda topado por la fecha de retiro del estudiante (`withdrawalDate`) si existe: no se puede crear uno que empiece despues del retiro, y una fecha de fin posterior se recorta automaticamente a la fecha de retiro — el tope se aplica tambien al momento de buscar el descuento vigente (no solo al crearlo), para que siga valiendo aunque `withdrawalDate` se defina o cambie despues. Integrado en frontend (`StudentDiscountsPanel.tsx`) el mismo dia, verificado en vivo contra el backend real.
